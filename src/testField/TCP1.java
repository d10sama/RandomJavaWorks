...
public class TCP1 extends Application{

	//游戏部
	Stage ps;
	private int mySerial=0;//我是白色
	int mpr,mpc;
	int lastr,lastc;
	GridPane cf;
	CFmid[] mids;
	...
	void MessageStage(String s)
	{
		...
		GridPane tmpp=new GridPane();
		Scene tmpc=new Scene(tmpp,300,100);
		Stage tmps=new Stage();
		tmps.setScene(tmpc);
		tmpp.add(tmpl, 0, 0);
		tmps.show();
	...
	void Turn(int mpr,int mpc,int b_w)
	{
		...
		default:
			...
			{
			case 0:
				this.left[mpr-1].Turn(b_w);
				break;
			case 14:
				this.rig[mpr-1].Turn(b_w);
				break;
			default:
	...
	class Listener extends Thread{
		...
		public void run() {
			try {
			Socket Listenersocket=new Socket("localhost",8003);
			while(!Listenersocket.isConnected())
				Listenersocket=new Socket("localhost",8003);
			System.out.println(Listenersocket.isConnected());
